...
export default function Clientes(){
  ...
  const [openHist, setOpenHist] = useState(null); // client_id
  ...
  const historyFor = (cid) => {
    ...
    const P = pays /* pode não ter client */ .map(p=>({ type:"pay", at:new Date(p.due_date).getTime(), title:`Pagamento: R$ ${Number(p.amount).toFixed(2)} (${p.status})`, sub:`${p.description} • Venc.: ${new Date(p.due_date).toLocaleDateString("pt-BR")}` }));
    ...
    <div className="stack">
      ...
      <div className="card">
        <div className="table-card">
          ...
          {list.map((c,i)=>(
            <div key={c.id} className="row" style={{justifyContent:"space-between", borderTop:i? '1px solid var(--border)':'none', paddingTop:i?8:0}}>
              <div style={{flex:1}}><strong>{c.name}</strong></div>
              <div className="muted">{c.phone||""}</div>
              <div className="muted">{c.address||""}</div>
              <button className="btn" onClick={()=>setOpenHist(c.id)}>Histórico</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <Modal open={!!openHist} onClose={()=>setOpenHist(null)} title={selected ? `Histórico — ${selected.name}` : "Histórico"}>
        <div className="stack">
          ...
          {selected && historyFor(selected.id).map((h,idx)=>(
            <div key={idx} style={{borderTop: idx? '1px solid var(--border)':'none', paddingTop: idx?8:0}}>
